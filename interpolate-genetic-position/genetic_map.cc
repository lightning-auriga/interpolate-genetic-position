...
void igp::genetic_map::query(const std::string &chr_query,
  ...
  if (verbose) {
    std::cerr << "query: chr is " << chr_query << ", pos is " << pos1_query
              << std::endl;
  }
  ...
  while (!_interface->eof()) {
    ...
    if (query_vs_lower_bound == EQUAL && query_vs_upper_bound == EQUAL) {
      if (verbose) {
        std::cerr << "\tsame chromosome, position comparing" << std::endl;
      }
      ...
      if (query_startpos_vs_lower_bound == 0 &&
        ...
        if (verbose) {
          std::cerr << "\t\tmatches lower boundary exactly: "
        ...
        if (verbose) {
          std::cerr << "\t\tmatches upper boundary exactly, so incrementing to "
                       "use that logic"
                    << std::endl;
        }
        _interface->get();
        ...
        if (verbose) {
          std::cerr << "\t\tbeginning of chromosome, before rate estimates "
                       "start; setting to 0"
                    << std::endl;
        }
        result->set_gpos(0.0);
        ...
        if (verbose) {
          std::cerr << "interpolated: gpos_lower_bound = "
        ...
        if (verbose) {
          std::cerr << "past current loaded window, incrementing range"
                    << std::endl;
        }
        _interface->get();
      }
      ...
      if (verbose) {
        std::cerr << "\tchromosome beyond end of range, setting to "
      ...
      if (verbose) {
        std::cerr << "\tno estimate for entire chromosome, setting to 0"
                  << std::endl;
      }
      result->set_gpos(0.0);
      result->set_endpos(pos2_query);
      ...
      if (verbose) {
        std::cerr << "\tgenetic map range needs incrementing" << std::endl;
      }
      _interface->get();
      if (query_vs_upper_bound == GREATER_THAN) {
        if (verbose) {
          std::cerr << "\t\ttwice!" << std::endl;
        }
        _interface->get();
      }
    } else {
  ...
  if (_interface->eof()) {
    if (query_vs_upper_bound == EQUAL) {
      ...
      if (verbose) {
        std::cerr << "\tfinal loaded data don't match chromosome, "
